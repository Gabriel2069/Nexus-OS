import { Activity, Archive, BarChart3, BookOpen, Brain, BriefcaseBusiness, CalendarCheck2, CalendarDays, CircleDollarSign, CircleHelp, Crosshair, Focus, GraduationCap, Inbox, ListChecks, Mountain, Repeat2, Sparkles } from 'lucide-react'
import type { LucideIcon } from 'lucide-react'
export type NavItem = { label: string; path: string; icon: LucideIcon; tone: string }
export type NavGroup = { label: string; items: NavItem[] }
export const navigation: NavGroup[] = [
  { label: 'Agora', items: [{ label: 'Agora', path: '/', icon: Crosshair, tone: 'blue' }, { label: 'Hoje', path: '/hoje', icon: CalendarDays, tone: 'cyan' }, { label: 'Calendário', path: '/calendario', icon: CalendarDays, tone: 'cyan' }, { label: 'Caixa de entrada', path: '/inbox', icon: Inbox, tone: 'amber' }]},
