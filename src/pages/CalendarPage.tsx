...
export function CalendarPage() {
  ...
  function selectDay(date: Date) {
    ...
    setSelectedDate(next); setExpandedEvent(null)
  ...
  function goToday(){const today=new Date();setCursor(today);selectDay(today)}
  function addForSelected(){emitUI('quickAdd',{type:'mission',dueDate:iso(selectedDate)})}
  async function toggleConditional(event: CalendarOccurrence){
    if(!userId||updatingEvent)return
    setUpdatingEvent(event.sourceId)
    const {error}=await supabase.from('calendar_commitments').update({is_optional:!event.isOptional,updated_at:new Date().toISOString()}).eq('id',event.sourceId).eq('user_id',userId)
    setUpdatingEvent(null)
    if(!error)await refresh()
  }
  const canToggle=(event:CalendarOccurrence)=>event.isOptional||/condicional|substitutiva/i.test(`${event.title} ${event.note??''}`)

  return <div className="page-stack calendar-page calendar-page--connected calendar-page--consultable">
    <section className="page-hero page-hero--cyan"><div><span className="eyebrow">Seu tempo</span><h1>Calendário</h1><p>Clique em qualquer dia para consultar o contexto completo. O calendário agora é também um lugar de decisão: você vê horários, missões, janelas livres e age dali mesmo.</p></div><button className="primary-button" onClick={addForSelected}><CalendarPlus size={16}/> Missão em {selectedIsToday?'hoje':compactDate(selectedDate)}</button></section>

    <section className="calendar-layout calendar-layout--consultable">
      <SurfaceCard className="calendar-card">
        <div className="calendar-toolbar"><strong>{monthLabel}</strong><div><button className="icon-button" onClick={()=>setCursor(new Date(year,month-1,1))} aria-label="Mês anterior"><ChevronLeft size={16}/></button><button className="text-button" onClick={goToday}>Hoje</button><button className="icon-button" onClick={()=>setCursor(new Date(year,month+1,1))} aria-label="Próximo mês"><ChevronRight size={16}/></button></div></div>
        <div className="calendar-weekdays">{weekday.map(day=><span key={day}>{day}</span>)}</div>
        <div className="calendar-grid">{cells.map(date=>{
          const key=iso(date);const fixed=occurrencesForDay(workspace.calendarCommitments,date).filter(event=>!['sleep','recovery'].includes(event.category));const missions=missionsByDate.get(key)??[];const today=key===iso(now);const selected=key===iso(selectedDate);const muted=date.getMonth()!==month;const total=fixed.length+missions.length
          return <button type="button" aria-pressed={selected} aria-label={`${fullDate(date)} · ${total} itens`} className={`calendar-day calendar-day--button ${today?'today':''} ${selected?'selected':''} ${muted?'muted':''}`} key={key} onClick={()=>selectDay(date)}>
            <span className="calendar-number">{date.getDate()}</span>
            <div className="calendar-events calendar-events--desktop">
              {fixed.slice(0,2).map(event=><span key={event.id} className={`calendar-event calendar-event--${categoryTone[event.category]??'slate'} ${event.isOptional?'is-optional':''}`}>{event.isOptional?'se necessário · ':`${time(event.start)} `}{event.title}</span>)}
              {missions.slice(0,1).map(mission=><span key={mission.id} className={`calendar-event rank-${mission.rank.toLowerCase()}`}>{mission.title}</span>)}
              {total>3&&<small>+{total-3}</small>}
            </div>
            <div className="calendar-dots" aria-hidden="true">{fixed.slice(0,4).map(event=><i className={`tone-bg-${categoryTone[event.category]??'slate'} ${event.isOptional?'is-optional':''}`} key={event.id}/>)}{missions.slice(0,2).map(mission=><i className={`rank-${mission.rank.toLowerCase()}`} key={mission.id}/>)}</div>
          </button>
        })}</div>
      </SurfaceCard>

      <aside className="calendar-agenda calendar-day-inspector">
        <SurfaceCard tone="blue" className="day-inspector-card">
          <div className="day-inspector__header"><div><span className="eyebrow">Dia selecionado</span><h2>{fullDate(selectedDate)}</h2></div><div><button className="icon-button" onClick={()=>shiftSelected(-1)} aria-label="Dia anterior"><ChevronLeft size={16}/></button><button className="icon-button" onClick={()=>shiftSelected(1)} aria-label="Próximo dia"><ChevronRight size={16}/></button></div></div>
          <div className="day-inspector__metrics"><span><strong>{selectedEvents.filter(event=>!event.isOptional).length}</strong> compromissos</span><span><strong>{selectedMissions.length}</strong> missões</span><span><strong>{Math.round(busyMinutes/60*10)/10}h</strong> ocupadas</span></div>
          <div className="day-inspector__actions"><button className="primary-button" onClick={addForSelected}><Plus size={15}/> Nova missão</button>{!selectedIsToday&&<button className="secondary-button" onClick={goToday}><RotateCcw size={15}/> Voltar a hoje</button>}</div>
        </SurfaceCard>

        <SurfaceCard eyebrow="Agenda" title={selectedEvents.length||selectedMissions.length?'Tudo deste dia':'Dia sem itens'}>
          <div className="day-detail-list">
            {selectedEvents.map(event=>{
              const expanded=expandedEvent===event.id
              return <article className={`day-detail-item ${event.isOptional?'is-optional':''}`} key={event.id}>
                <button className="day-detail-item__main" onClick={()=>setExpandedEvent(expanded?null:event.id)}>
                  <span className={`agenda-category tone-${categoryTone[event.category]??'slate'}`}><Clock3 size={14}/></span>
                  <div><small>{isTimed(event)?`${time(event.start)}–${time(event.end)}`:'dia inteiro'} · {categoryName[event.category]??event.category}</small><strong>{event.title}</strong></div>
                  <ArrowRight className={expanded?'is-rotated':''} size={14}/>
                </button>
            ...
            {selectedMissions.map(mission=><article className="day-mission-item" key={mission.id}><span className={`rank-chip rank-${mission.rank.toLowerCase()}`}>{mission.rank}</span><div><strong>{mission.title}</strong><small>{mission.duration_minutes?`${mission.duration_minutes} min · `:''}{mission.priority}</small></div><div className="day-mission-actions">{selectedIsToday&&<button className="icon-button" onClick={()=>navigate(`/foco?minutes=${mission.duration_minutes??45}&mission=${mission.id}`)} aria-label={`Focar em ${mission.title}`}><Focus size={14}/></button>}<button className="icon-button" onClick={()=>void completeMission(mission.id)} aria-label={`Concluir ${mission.title}`}><CheckCircle2 size={15}/></button></div></article>)}
            {!selectedEvents.length&&!selectedMissions.length&&<div className="empty-compact"><CalendarDays size={17}/>Nada marcado. Esse dia pode continuar livre.</div>}
          </div>
        </SurfaceCard>

        <SurfaceCard tone="green" eyebrow="Espaço disponível" title={windows.length?'Janelas livres':'Sem janela longa'}>
          <div className="free-window-list">{windows.slice(0,4).map((window,index)=>{const preset=focusPreset(window.minutes);const current=selectedIsToday&&index===0&&window.start<=now&&window.end>now;return <div className="free-window" key={`${window.start.toISOString()}-${window.end.toISOString()}`}><div><strong>{time(window.start)}–{time(window.end)}</strong><span>{window.minutes} min livres</span></div>{current&&<button className="secondary-button" onClick={()=>navigate(`/foco?minutes=${preset}`)}><Focus size={14}/> Foco {preset}</button>}</div>})}{!windows.length&&<p className="panel-copy">Entre 7h e 22h não há um intervalo livre de pelo menos 25 minutos neste dia.</p>}</div>
        </SurfaceCard>

        <SurfaceCard tone="slate" eyebrow="Adiante" title="Próximos compromissos"><div className="agenda-list">{upcomingCalendar.map(event=><button className={`agenda-item agenda-item--calendar agenda-item--button ${event.isOptional?'agenda-item--optional':''}`} key={event.id} onClick={()=>selectDay(event.start)}><span className={`agenda-category tone-${categoryTone[event.category]??'slate'}`}><CalendarDays size={14}/></span><div><strong>{event.title}</strong><small>{compactDate(event.start)} · {event.isOptional?'se necessário':time(event.start)}</small></div></button>)}</div></SurfaceCard>
      </aside>
    </section>
  </div>
}
