  ...
  return <div className="page-stack">
    ...
    <section className="calendar-layout">
      <SurfaceCard className="calendar-card">
        <div className="calendar-toolbar"><div><button className="icon-button" onClick={() => setCursor(new Date(year,month-1,1))}><ChevronLeft size={16}/></button><button className="text-button" onClick={() => setCursor(new Date())}>Hoje</button><button className="icon-button" onClick={() => setCursor(new Date(year,month+1,1))}><ChevronRight size={16}/></button></div><strong>{monthLabel}</strong></div>
        <div className="calendar-weekdays">{weekday.map((day) => <span key={day}>{day}</span>)}</div>
        <div className="calendar-grid">{cells.map((date) => { const key=iso(date); const items=missionsByDate.get(key) ?? []; const today=key===iso(new Date()); const muted=date.getMonth()!==month; return <div className={`calendar-day ${today?'today':''} ${muted?'muted':''}`} key={key}><span className="calendar-number">{date.getDate()}</span><div className="calendar-events">{items.slice(0,3).map((mission) => <span key={mission.id} className={`calendar-event rank-${mission.rank.toLowerCase()}`}>{mission.title}</span>)}{items.length>3 && <small>+{items.length-3}</small>}</div></div> })}</div>
      </SurfaceCard>
