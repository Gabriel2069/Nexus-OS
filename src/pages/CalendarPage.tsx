...
export function CalendarPage() {
  ...
  return <div className="page-stack calendar-page">
    <section className="page-hero page-hero--cyan"><div><span className="eyebrow">Datas e prazos</span><h1>Calendário</h1><p>Veja quando as missões vencem e o que está chegando nos próximos dias.</p></div><button className="primary-button" onClick={() => emitUI('quickAdd')}><Plus size={16}/> Novo</button></section>
    <section className="calendar-layout">
      ...
      <aside className="calendar-agenda"><SurfaceCard tone="blue" eyebrow="Próximos prazos" title="Agenda"><div className="agenda-list">{upcoming.length ? upcoming.map((mission) => <div className="agenda-item" key={mission.id}><span className={`rank-chip rank-${mission.rank.toLowerCase()}`}>{mission.rank}</span><div><strong>{mission.title}</strong><small><Clock3 size={11}/> {mission.due_at ? new Intl.DateTimeFormat('pt-BR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}).format(new Date(mission.due_at)) : 'Sem prazo'}</small></div></div>) : <div className="empty-compact"><CalendarDays size={18}/><span>Nenhum prazo definido.</span></div>}</div></SurfaceCard></aside>
    </section>
  </div>
}
