...
export function CalendarPage() {
  ...
  const now = new Date()
  const upcomingCalendar = useMemo(()=>upcomingOccurrences(workspace.calendarCommitments,now,16).filter(event=>!['sleep','recovery'].includes(event.category)).slice(0,10),[workspace.calendarCommitments])
        ...
        <div className="calendar-grid">{cells.map((date) => {
          ...
          return <div className={`calendar-day ${today?'today':''} ${muted?'muted':''}`} key={key}>
            ...
            <div className="calendar-events calendar-events--desktop">
              {fixed.slice(0,2).map((event)=><span key={event.id} className={`calendar-event calendar-event--${categoryTone[event.category]??'slate'} ${event.isOptional?'is-optional':''}`} title={`${event.title} · ${event.isOptional?'se necessário · ':''}${time(event.start)}`}>{event.isOptional?'se necessário · ':`${time(event.start)} `}{event.title}</span>)}
            ...
            <div className="calendar-dots" aria-label={`${total} itens no dia`}>{fixed.slice(0,4).map(event=><i className={`tone-bg-${categoryTone[event.category]??'slate'} ${event.isOptional?'is-optional':''}`} key={event.id}/>)}{missions.slice(0,2).map(mission=><i className={`rank-${mission.rank.toLowerCase()}`} key={mission.id}/>)}</div>
          </div>
        })}</div>
      </SurfaceCard>
      <aside className="calendar-agenda">
        <SurfaceCard tone="blue" eyebrow="Próximos" title="Agenda e marcadores"><div className="agenda-list">{upcomingCalendar.length ? upcomingCalendar.map((event)=><div className={`agenda-item agenda-item--calendar ${event.isOptional?'agenda-item--optional':''}`} key={event.id}><span className={`agenda-category tone-${categoryTone[event.category]??'slate'}`}><CalendarDays size={14}/></span><div><strong>{event.title}</strong><small><Clock3 size={11}/>{new Intl.DateTimeFormat('pt-BR',{weekday:'short',day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit',timeZone:'America/Sao_Paulo'}).format(event.start).replace('.','')}{event.isOptional&&<em className="agenda-optional">se necessário</em>}</small></div></div>):<div className="empty-compact"><CalendarDays size={18}/><span>Sem compromissos próximos.</span></div>}</div></SurfaceCard>
