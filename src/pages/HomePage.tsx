import { ArrowRight, CalendarClock, Check, Clock3, Coins, Focus, GraduationCap, Sparkles, Trophy } from 'lucide-react'
...
const categoryLabel: Record<string, string> = {
  school: 'Escola', class: 'Aula', study: 'Estudo', exam: 'Prova', health: 'Saúde', recovery: 'Pausa', sleep: 'Sono',
}
...
export function HomePage() {
  ...
  const name = workspace.profile?.display_name?.split(' ')[0] || 'Gabriel'
  ...
  return <div className="page-stack now-page">
    ...
    <section className="now-support-grid">
      <SurfaceCard tone="blue" eyebrow="Hoje" title="O dia em ordem">
        <div className="day-route">
          {context.today.filter((event) => !['sleep', 'recovery'].includes(event.category)).map((event) => {
          ...
          {!context.today.length && <div className="empty-compact">Nenhum compromisso fixo hoje.</div>}
        </div>
      </SurfaceCard>
      ...
      <div className="now-side-stack">
        <SurfaceCard tone="violet" eyebrow="Rotina gamificada" title="Só o essencial">
          <div className="essential-loop">
            ...
            <button className={todayFocus > 0 ? 'done' : ''} onClick={() => context.suggestion.actionPath?.startsWith('/foco') ? navigate(context.suggestion.actionPath) : navigate('/foco')}><span>{todayFocus > 0 ? <Check size={14}/> : '2'}</span><div><strong>Um bloco que importa</strong><small>{todayFocus > 0 ? `${todayFocus} min registrados` : 'quando o calendário abrir espaço'}</small></div></button>
        ...
        <SurfaceCard tone="amber" eyebrow="Próxima pressão" title={context.nextExam?.title ?? 'Sem prova próxima'}>
          <div className="exam-glance">
            <GraduationCap size={20}/>
            <div>{context.nextExam ? <><strong>{context.daysToExam === 0 ? 'Hoje' : context.daysToExam === 1 ? 'Amanhã' : `em ${context.daysToExam} dias`}</strong><span>{new Intl.DateTimeFormat('pt-BR', { weekday: 'short', day: '2-digit', month: 'short', timeZone: 'America/Sao_Paulo' }).format(context.nextExam.start).replace('.', '')}</span></> : <><strong>Calendário respirando</strong><span>não invente urgência</span></>}</div>
          </div>
        </SurfaceCard>
      </div>
    </section>
    ...
    <section className="now-bottom-grid">
      <SurfaceCard eyebrow="Depois" title={context.later?.title ?? 'O resto pode esperar'}>
        <p className="panel-copy">{context.later?.detail ?? 'Quando este bloco terminar, volte ao Início e o Nexus recalcula o próximo passo.'}</p>
      </SurfaceCard>
      <SurfaceCard tone="green" eyebrow="Vida fora da lista" title="Tempo livre continua sendo tempo livre">
        <p className="panel-copy">Amigos, família, cabelo, hobby e descanso entram quando o calendário abre espaço. Eles não precisam virar uma fila diária de tarefas para contar.</p>
      </SurfaceCard>
      <SurfaceCard tone="violet" eyebrow="Progressão" title={`Nível ${workspace.profile?.level ?? 1}`}>
        <div className="now-progress"><Trophy size={18}/><div><strong>{workspace.profile?.xp_total ?? 0} XP</strong><span><Coins size={12}/>{workspace.profile?.nexus_coins ?? 0} coins</span></div></div>
      </SurfaceCard>
    </section>
