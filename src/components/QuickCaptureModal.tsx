...
export function QuickCaptureModal() {
  const { addMission, workspace } = useNexus()
  ...
  async function submit(event: FormEvent) {
    ...
    await addMission(title, {
      ...
      project_id: type === 'mission' && projectId ? projectId : null,
    })
    setSaving(false)
    setTitle('')
    setDueDate('')
  ...
  return <div className="overlay" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
    <section className="quick-modal" role="dialog" aria-modal="true">
      <div className="modal-title"><div><span className="eyebrow">Captura rápida</span><h2>Coloque no sistema.</h2></div><button className="icon-button" onClick={() => setOpen(false)}><X size={17} /></button></div>
      ...
      <form onSubmit={submit} className="quick-form">
        ...
        {type === 'mission' && <>
          ...
          <label className="quick-project-label">Projeto <span>opcional</span></label>
          <select value={projectId} onChange={(e)=>setProjectId(e.target.value)}><option value="">Sem projeto</option>{workspace.projects.filter(project=>!['Concluído','Arquivado'].includes(project.status)).map(project=><option value={project.id} key={project.id}>{project.name}</option>)}</select>
